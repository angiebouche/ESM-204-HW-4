---
title: "ESM 204 HW 4"
author: "Angie Bouche"
date: "June 5, 2019"
output: html_document
---

```{r setup, include=FALSE}
library(tidyverse)
library(stargazer)
library(dplyr)
library(ggplot2)
library(Hmisc)
library(RColorBrewer)

emissions <- read_csv("HW4Data.csv")
```

## Assumptions
```{r}
# Current carbon emissions in the sectors are: (A=180, B=200, C=220, and D=300).
# Sectors A, B, and C are in country X. Sector D is in a different country, Y.
# Prices are in $/ton and quantities are in tons.
```

##Part 1 Regressions
```{r}

model_a1 <- lm(MC_A ~ Abatement, data = emissions, family="linear")
summary(model_a1) #r square .91

#y = 0.57684 x - 8.64448

model_b1 <- lm(MC_B ~ Abatement, data = emissions, family="linear")
summary(model_b1) #r sq .56

#y = 0.19874 x + 9.31770

model_c1 <- lm(MC_C ~ Abatement, data = emissions, family="linear")
summary(model_c1) #r sq .87

#y = 0.78383 x - 11.65503

model_d1 <- lm(MC_D ~ Abatement, data = emissions, family="linear")
summary(model_d1)#r sq .54

#y = 0.25993 x + 9.68751
```

```{r stargazer, results = 'asis'}
# Stargazer table
stargazer_lm <- stargazer(model_a1, model_b1, model_c1, model_d1, type="html", title="Regression Results")

```
###Plot
```{r}
ggplot(emissions, aes(x = Abatement, y = MC_A)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "indianred1")+
  ylab("Marginal Cost of Abatement (Sector A)")+
  theme_classic()

ggplot(emissions, aes(x = Abatement, y = MC_B)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "olivedrab3")+
  ylab("Marginal Cost of Abatement (Sector B)")+
  theme_classic()

ggplot(emissions, aes(x = Abatement, y = MC_C)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "turquoise3")+
  ylab("Marginal Cost of Abatement (Sector C)")+
  theme_classic()

ggplot(emissions, aes(x = Abatement, y = MC_D)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "orchid3")+
  ylab("Marginal Cost of Abatement (Sector D)")+
  theme_classic()


```
## Plot 2
```{r}

```


##Part 2 Demand
```{r}

#find yint
#y = 0.57684 x - 8.64448
-8.64448 + (0.57684*180)
#(0, 95.18672)

#y = 0.19874 x + 9.31770
9.31770 + (0.19874*200)
#(0, 49.0657)

#y = 0.78383 x - 11.65503
-11.65503 + (0.78383*220)
#(0, 160.7876)

#y = 0.25993 x + 9.68751
9.68751 + (0.25993*300)
#(0, 87.66651)

#Sector C is willing to pay the most for the first unit, because they have the largest y intercept??


#no idea what this does but it was in the other code - making a dataset of points along the demand curve?
demanda<-Hmisc::bezier(x = c(0, 180), y = c(95.18672, 0)) %>% as_data_frame()
#y = -0.57684 x + 95.18672
demandb<- Hmisc::bezier(x = c(0, 200), y = c(49.0657, 0)) %>% as_data_frame()
#y = -0.19874 x + 49.0657
demandc<-Hmisc::bezier(x = c(0, 220), y = c(160.7876, 0)) %>% as_data_frame()
#y = -0.78383 x + 160.7876
demandd<- Hmisc::bezier(x = c(0, 300), y = c(87.66651, 0)) %>% as_data_frame()
#y = -0.25993 x + 87.66651

demanda$type<- rep("Sector A", length(demanda$x))
demandb$type<- rep("Sector B", length(demandb$x))
demandc$type<-rep("Sector C", length(demandc$x))
demandd$type<- rep("Sector D", length(demandc$x))

#merging all the demand curves together
all<- rbind(demanda, demandb, demandc, demandd)
all

ggplot(all, aes(x=x, y=y, color=type))+
  #tiff('demand.tiff', units="in", width=10, height=5, res=300)+
  geom_line(size=1)+
  labs(x="Carbon Emissions (tons)", y="Price ($) per Ton", title="Estimated Demand Curve for Carbon Emissions")+
  theme_classic()+
  scale_x_continuous(expand=c(0,0))+
  scale_y_continuous(expand=c(0,0))+
  theme(legend.title=element_blank())+
  theme(plot.title = element_text(hjust = 0.5))

```


#$Part 3: Country X
# For each of the following policies derive: 1. the total cost of meeting the target in country X 2. the cost or benefit to each sector and 3. the tax revenue generated
###  a. Carbon Cap
####No tax revenue generated


```{r}
#MC when 100 tons are abated
MCa100 = (-0.57684*100) + 95.18672 
#37
MCb100= (-0.19874*100) + 49.0657
#29
MCc100= (-0.78383*100) + 160.7876
#82

#Find area under demand curve for x=0-100. Add area of rectange and triangle together
TCa100 = (100*MCa100)+((95.18672- MCa100)*100*.5) # 6634.5
TCb100 = (100*MCb100)+((49.0657- MCb100)*100*.5) # 3912.9
TCc100 = (100*MCc100)+((160.7876- MCc100)*100*.5) # 12159.6

Tc100 = TCa100 +TCb100+ TCc100 # 22707.0

```

### b. Carbon Tax
```{r}
#Aggreate  MC for tax
Aggregate_Tax = (617.0287 - 300) /8
# x= 8y-15.20 or y=(x+15.20)/8
# MC_abc=(x+15.20)/8 (Sum of MC for a, b, and c)
MC_abc=(300+15.20)/8 # When abatement = 300

MC_abc

# Revenue tax
# Equal tax price times emissions
revenue_tax= 300*MC_abc

# Marginal cost equations for each sector
MC_a= function(x){
  (0.57684*x - 8.64448)
}

MC_b= function(x){
  (0.19874 * x + 9.31770)
}

MC_c= function(x){
  (0.78383 * x - 11.65503)
}

MC_d= function(x){
  (0.25993 * x + 9.68751)
}

# Cost for each sector of country X
cost_a_tax= integrate(MC_a, 0, 84.3)$value # 1320.9
cost_b_tax=integrate(MC_b, 0, 150.4)$value # 3649.1
cost_c_tax= integrate(MC_c, 0, 66.7)$value # 966.2

# Total cost of tax
cost_total_tax = cost_a_tax + cost_b_tax + cost_c_tax # 5936.3

```
$MC_agg={}{}$
### c. Cap and Trade
```{r}
# to stop trade two things need to happen
# 1. every sector needs to have the same abatement cost
# 2. total abatement needs to equal 300

# need to make assumption about 
# units in terms of abatement

abate_a = (MC_abc+8.65)/0.57 #84.3 
abate_b = (MC_abc-9.32)/0.2 #150.4
abate_c = (MC_abc+12.66)/0.78 #66.7

# Check abatement in each sector equals 300
abate_abc = abate_a +abate_b + abate_c

# Units bought by each sector
# Each sector needed needed to abate 100 units
bought_a = 100-84.3
bought_b = 100-150.4
bought_c =100-66.7

# Find cost per sector
cost_a_trade = (bought_a*MC_abc) + cost_a_tax # 1939.5
cost_b_trade = (bought_b*MC_abc) + cost_b_tax # 1663.1
cost_c_trade = (bought_c*MC_abc) + cost_c_tax # 2278.2

# Find total cost
cost_total_trade = cost_a_trade + cost_b_trade + cost_c_trade # 5881.1
```

##Part 4
```{r}
# Yes because country's Y MC is lower than sectors A and C of country X
```

##Part 5 
### A- With a cap and trade market, country X will reduce its emissions by 300 tons overall, leaving 300 tons of carbon emitted. This corresponds with 300 tons of local air pollution. For country Y, if they aren't in a cap and trade program they will continue to emit 300 tons of carbon, which corresponds with 300 tons of air pollution there. 

```{r}
# b.
# First need to calculate how much each sector would emit
# x = 11.88y-52.46
# MC_abcd= (x+52.46)/11.88
MC_abcd= (300+52.46)/11.88

abate_a2 = (MC_abcd+8.65)/0.57 #67.2
abate_b2 = (MC_abcd-9.32)/0.2 #101.7
abate_c2 = (MC_abcd+12.66)/0.78 #54.3
abate_d2 = (MC_abcd-9.69)/0.26 #76.8

# Check total abatement
abate_abcd = abate_a2+abate_b2+abate_c2+abate_d2

# c. Trade is even more beneficial for country Y because it is also getting health benefits from reducing 76.8 tons of carbon
```




